...
if(!class_exists('AIO_Icon_Manager'))
{
	class AIO_Icon_Manager
	{
		...
		{
			$this->assets_js = plugins_url('js/',__FILE__);
			$this->assets_css = plugins_url('css/',__FILE__);
			...
			$this->vc_fonts_dir = trailingslashit(plugin_dir_path( __FILE__ )).'fonts/';
			...
			add_action('admin_enqueue_scripts',array($this,'aio_front_scripts'));
		}
		function aio_front_scripts()
		{
			// enqueue css files on frontend
			wp_enqueue_style('aio-icon-manager',$this->assets_css.'icon-manager.css');
		...
		function admin_scripts()
		{
			...
			wp_enqueue_media();
			wp_enqueue_style('aio-icon-manager-admin',$this->assets_css.'icon-manager-admin.css');
		}// end AIO_admin_scripts
			...
			foreach($fonts as $font => $info)
			{
				...
				if(!empty($icon_set))
				{
					...
					if($n != 1)
						...
						$output .= '<button class="button button-secondary button-small smile_del_icon" data-delete='.$font.' data-title="Delete This Icon Set">Reset Icon Set</button>';
					$output .= '</h3>';
